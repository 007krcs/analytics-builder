...
// ─── Demo shell (lazy-loaded by App.tsx) ──────────────────────
//
// Everything below the landing page lives here so the landing chunk ships
// without the analytics engine, dnd-kit, or recharts. App.tsx lazy-imports
// this module the first time the user launches the demo.

export default function DemoShell({ onBack }: { onBack: () => void }) {
    ...
    <div className="app">
      ...
      <div className="lp-back-bar" role="region" aria-label="Demo session header">
        ...
        <button
          ...
          onClick={() => { onBack(); window.scrollTo({ top: 0 }); }}
      ...
      <main className="app-main" role="main" id="main-content">
        ...
        {activeTab === 'ask' && salesDataset && (
          <AskPanel
            ...
            // Register uploads with the engine so the Pivot/Chart/KPI tabs can
            // explore the same data via their dataset selectors.
            onDatasetUploaded={(ds) => loadDataset('uploaded', ds.name, ds.rows)}
          />
        )}
...
const SCHEDULES_LS_KEY = 'analytix-schedules';
...
function ReportSchedulerWidget() {
  // Schedules survive reloads via localStorage.
  const [entries, setEntries] = useState<ScheduleEntry[]>(() => {
    try {
      const raw = localStorage.getItem(SCHEDULES_LS_KEY);
      if (raw) return JSON.parse(raw) as ScheduleEntry[];
    } catch { /* corrupted → defaults */ }
    return DEFAULT_SCHEDULES;
  });

  useEffect(() => {
    try { localStorage.setItem(SCHEDULES_LS_KEY, JSON.stringify(entries)); } catch { /* quota */ }
  }, [entries]);
...
const CANVAS_LS_KEY = 'analytix-canvas-layout';

// Restore the saved layout, or pre-place 3 widgets on first-ever visit.
...
function bootstrapCanvas() {
  ...
  try {
    ...
    if (raw) {
      CANVAS_ENGINE.loadLayout(JSON.parse(raw)); // versioned + migrated
      return;
    }
  } catch { /* corrupted layout → fall through to defaults */ }
...
// Persist every layout change so the canvas survives reloads.
CANVAS_ENGINE.subscribe((layout) => {
  try { localStorage.setItem(CANVAS_LS_KEY, JSON.stringify(layout)); } catch { /* quota */ }
});
